...
def build_default_seats() -> list[Seat]:
    ...
    - 第二行多一個座位（設在第 7 列第 2 行）→ 總共 37 個座位
    ...
    # 第二行多出一個座位（示例放在第 7 列第 2 行）
    seats.append(Seat(id=seat_id, row=7, col=2))  # 這裡如要調整位置可改 row
    # 總共 37 個座位
    return seats
...
def build_adjacency_maps(seats: list[Seat]):
    """
    建立：
    - adjacent_lr: 只算左右相鄰（相同列，col 差 1）→ 用於「相鄰」條件
...
def check_partial_constraints(
    ...
    - 不相鄰（寬鬆版）：用 adjacent_lr（左右，其實等於「左右不鄰」）
      若你想改成「前後左右不鄰」，可以改成另一個 adjacency map。
    """
...
def main():
    ...
    st.markdown(
        ...
        - 支援每位學生：
          - 限制只能坐在哪些「列」（前後）
          - 限制只能坐在哪些「行」（左右）
        - 支援指定兩位學生「相鄰（只算左右）」或「不相鄰（九宮格 / 前後左右）」
    ...
    col_a, col_b, col_type, col_btn = st.columns([1, 1, 1, 1])
    with col_a:
        rule_a = st.selectbox("座號 A", options=student_ids, key="rule_a")
    with col_b:
        rule_b = st.selectbox("座號 B", options=student_ids, key="rule_b")
    with col_type:
        rule_type = st.selectbox("類型", options=["相鄰", "不相鄰"], key="rule_type")

    with col_btn:
        if st.button("新增條件"):
            if rule_a == rule_b:
                st.warning("⚠️ A 與 B 不可相同。")
            else:
                st.session_state["adjacency_rules"].append(
                    {"座號A": rule_a, "座號B": rule_b, "類型": rule_type}
                )
    ...
    if layouts:
        ...
        st.caption("提示：若教室實際座位位置與預設不同，可修改程式中的 build_default_seats()。")
